...
def evaluateXpath( String xml, String xpathQuery) {
	...
	for (int i = 0; i < compEmpList.getLength(); i++) 
		{
			xmlString.append("Person ID External: " + xpath.evaluate("log/log_item/person_id_external/text()",compEmpList.item(i),XPathConstants.STRING) + "\n")
